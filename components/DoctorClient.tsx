...
export default function DoctorClient({ lang }: { lang: Language }) {
  ...
  /** Queue = stored appointments only; completed ones are filtered out. */
  const activeQueue = queue.filter(
    (patient) =>
      !patient.appointmentId ||
      getAppointments().find(
        (appointment) => appointment.id === patient.appointmentId
      )?.status !== "Completed"
  );
  ...
  useEffect(() => {
    setQueue(mergeQueue());
    setCompletedToday(getCompletedOn(localToday()).length);
  ...
  /**
   * "Send to Patient" lifecycle on the saved card:
   * Sending... (500ms) → ✓ Sent (2s) → mark Completed + drop from queue
   * + auto-select the next waiting patient. The prescription record itself
   * was already persisted by handleSave() — the saved card only exists
   * after a successful save, so no duplicate record is written here.
   */
  function handleSendToPatient(patient: Patient) {
    if (sendState !== "idle") return;
    setSendState("sending");
    window.setTimeout(() => {
      setSendState("sent");
      showToast(t.doctor.sentToastNamed.replace("{name}", patient.name));
      // (d) After the ✓ Sent state, auto-complete the patient.
      window.setTimeout(() => {
        setSendState("idle");
        completePatient(patient, false);
      }, 2000);
    }, 500);
  }
  ...
  /**
   * Mark a patient as seen/completed and remove them from the active queue,
   * auto-selecting the next waiting patient. Appointment-backed patients are
   * stamped Completed in the shared store (so /appointments reflects it and
   * they count towards "Completed Today").
   */
  function completePatient(patient: Patient, announce = true) {
    if (patient.appointmentId) {
      updateAppointmentStatus(patient.appointmentId, "Completed");
      setCompletedToday(getCompletedOn(todayStr).length);
    }
    const remaining = activeQueue.filter((item) => item.id !== patient.id);
    setQueue(remaining);
    ...
    if (announce) showToast(t.doctor.completedToast);
  }
        ...
        <section className="h-fit rounded-3xl bg-white p-5 shadow-sm ring-1 ring-slate-200 lg:sticky lg:top-6">
          ...
          <ul className="mt-4 space-y-2">
            ...
            {activeQueue.length === 0 && (
              <li className="rounded-2xl border-2 border-dashed border-slate-200 px-4 py-6 text-center text-xs font-semibold text-slate-400">
                {t.doctor.queueEmpty}
              </li>
            )}
          </ul>

          {/* Completed today — collapsed by default, click to expand names. */}
          {completedToday > 0 && (
            <div className="mt-4 border-t border-slate-100 pt-3">
              <button
                type="button"
                onClick={() => setShowCompletedToday((open) => !open)}
              ...
              <div className="mt-6 flex flex-col items-center justify-between gap-4 sm:flex-row">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <button
                    ...
                    className={`inline-flex items-center gap-1.5 rounded-xl px-5 py-2.5 text-sm font-extrabold text-white shadow-lg transition ${
                      sendState === "sent"
                  ...
                  >
                    ...
                    {sendState === "sending" && t.doctor.sending}
                    {sendState === "sent" && t.doctor.sent}
                  </button>
                </div>
              </div>
            </div>
